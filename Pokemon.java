...
public abstract class Pokemon implements Comparable<Pokemon> {
   ...
   **/
   public Pokemon(String species, String name, 
      ...
      if (name.trim().length() < 1) {
         throw new PokemonException("The pokemon must have a name!");
      }
      //for initial level
  ...
  * @exception PokemonException if the name is empty
  */
   public void setName(String newName) throws PokemonException {
      if (newName.isEmpty()) {
         throw new PokemonException("The pokemon must have a vaild set name !");
      } else {
         name = newName;
      }
   }
...
/**
   * This check if two pokemons are equal.
   *
   * @param p2 Is The 2nd Pokemon.
   * @return boolean
   */
   public boolean equals(Pokemon p2) {
      if (this.number != p2.getNumber()
           || !this.name.equals(p2.getName())
           || this.hP != p2.getHP()
           || this.cP != p2.getCP()
           || !this.fastAttack.equals(p2.getFastAttack())
           || !this.specialAttack.equals(p2.getSpecialAttack())) {
         return false; 
      }
      return true; 
   } // close equals
...
/**
   ...
   */
   public int compareTo(Pokemon p2) {
      ...
      int result = 0;
      //difference between numberic variables
      int numDiff = 0;
      ...
      String fastAk1 = this.getFastAttack();
      String fastAk2 = p2.getFastAttack();
      String specialAk1 = this.getSpecialAttack();
      String specialAk2 = p2.getSpecialAttack();
      ...
      if (numDiff < 0) {
         ...
         // if the number is the same, compare the name
         result = name1.compareTo(name2);
         return result;
      } else {
       // if the name is the same, compare the hp
         numDiff = hP1 - hP2;
      }
      if (numDiff > 0) {
         result = 1;
      ...
      // if the hp is the same, compare the cp
         numDiff = cP1 - cP2;
      } 
      if (numDiff > 0) {
         result = 1;
         ...
         result = -1;
      } else if (fastAk1.compareTo(fastAk2) != 0) {
     // if the cp is the same, compare the fastattack
         result = fastAk1.compareTo(fastAk2);
      } else if (specialAk1.compareTo(specialAk2) != 0) {
        // if the fastattack is the same, 
        //compare the special attack                    
         result = specialAk1.compareTo(specialAk2);
      } else {
         result = 0;
      }
      return result;
